...
public class CalendarPage extends JPanel {
    ...
    public CalendarPage(Router router, LogicController logic) {
        ...
        datePickerButton.addActionListener(e -> {
            ...
            if (result == JOptionPane.OK_OPTION) {
                this.selectedMonth = monthBox.getSelectedIndex() + 1;
                this.selectedYear = (int) yearSpinner.getValue();
                updateDatePickerLabel();
                drawCalendar(router);
            }
        });
    ...
    private void updateDatePickerLabel() {
        String monthName = Month.of(this.selectedMonth).getDisplayName(TextStyle.FULL, Locale.ENGLISH);
        datePickerButton.setText(monthName + " " + this.selectedYear);
    }
    ...
    private void drawCalendar(Router router) {
        ...
        YearMonth yearMonth = YearMonth.of(this.selectedYear, this.selectedMonth);
        ...
        for (int day = 1; day <= daysInMonth; day++) {
            ...
            if (this.selectedYear == today.getYear() && this.selectedMonth == today.getMonthValue() && day == today.getDayOfMonth()) {
                dayButton.setFont(new Font("SansSerif", Font.BOLD, 16));
            } 

            int selectedDay = day;
            dayButton.addActionListener(e -> {
                LocalDate selectedDate = LocalDate.of(this.selectedYear, this.selectedMonth, selectedDay);
